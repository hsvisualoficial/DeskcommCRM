...
| `20260706220000` | `0028_whatsapp_media_bucket` | Setup self-host: cria o bucket privado `whatsapp-media` (referenciado pela RPC de expurgo LGPD `fn_lgpd_cascade_redact` mas nunca criado por nenhuma migration/baseline anterior). Acesso via URLs assinadas server-side. |
| `20260722000000` | `0029_lead_scoring_sdr` | EPIC-SDR wave 1 (B2C + Prestadores de Serviço): scoring de leads. Adiciona `contacts.score` (int 0-100), `contacts.priority_tag` (COLUNA GERADA: >=70 HOT / >=40 WARM / senão COLD — doutrina "Calcular", sem trigger) e `contacts.custom_fields` (jsonb, dados por segmento). Constraint `contacts_score_range` (DO-guard idempotente) + índice `contacts_org_priority_idx`. Motivo de perda do Kanban reutiliza `crm_leads.lost_reason` (já existente); a Fase 1 apenas passa a coletá-lo no drag-to-lost via UI. |
| `20260722120000` | `0030_snippets_and_notes` | EPIC-SDR wave 2 (produtividade do Inbox): `chat_snippets` (respostas rápidas por org, atalho único `(organization_id, shortcut)`, content com variáveis `{{...}}`) e `conversation_notes` (notas internas por conversa, `source` manual/ai_summary — geradas pelo "Resumir atendimento com IA"; nunca vão ao WhatsApp). Ambas RLS via `fn_user_org_ids()`. Seed genérico+idempotente de 5 atalhos iniciais por org (`on conflict do nothing`). |
| `20260722140000` | `0031_scheduled_messages` | EPIC-SDR wave 3 (follow-up/cadência): `scheduled_messages` — fila de mensagens WhatsApp agendadas (follow-up manual do vendedor ou passo de régua de cadência). Consumida pelo cron `scheduled-dispatch` que envia via WAHA reusando `sendMessageHandler`. `status` pending/sent/failed/canceled, `source` manual/cadence, dedupe de cadência por `(contact_id, trigger_ref)` parcial em pending. Cadência por etapa configurada em `crm_pipelines.settings.cadence` (jsonb, sem tabela nova). RLS via `fn_user_org_ids()`. |
| `20260722160000` | `0032_sdr_first_response_rpc` | EPIC-SDR wave 3 (dashboard): `fn_sdr_first_response_avg_seconds(org, since)` — média em segundos entre a 1ª mensagem do cliente e a 1ª resposta do atendente por conversa, na janela. SECURITY INVOKER (RLS de `messages` aplica). Usada pela KPI bar do Kanban (Média de Tempo de Primeira Resposta). |
